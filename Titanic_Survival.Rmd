---
title: "Titanic Survival Part 1: Exploratory Data Analysis"
...
* [Overview of the Project](#overview-link) 
...
Kaggle is a platform for data science, hosting data, competitions and more. The Titanic Competition is designed as an introduction to Kaggle competitions, machine-learning, or both. Here is a description from Kaggle:
...
In Exploratory Data Analysis we use summary tables and plots (data visualizations) to explore various facets of data. An excellent tool for this type of work is R programming and RStudio, the open-source IDE for R. With RStudio we can weave code and text using RMarkdown and a package called knitr, creating a reproducible report: given the data, the code, and R or RStudio, anyone can reproduce a report's analysis.

This can all be done using Python and Jupyter Notebooks, or a variety of proprietary tools. While there are many **R vs Python** blogs detailing the virtues of each language, in my current understanding and skill level, I believe R and RStudio to be superior when conducting quick exploratory plots, if only because one can interactively print to the console in R, but also because its base plotting function `plot()` will take almost any type of data and produce some kind of result. R was developed "by statisticians, for statisticians" and comes with many built-in statistical properties, not requiring any extra module or library.

In this report, I use the famous `ggplot2` package by Hadley Wickham as well, since it has many graphical features, which perhaps goes a bit beyond quick and dirty plotting for EDA, but also aim at reporting.

## Machine Learning and Python

This project is also my response to Exercise 3 at the end of Chapter 3 of Aurelien Geron's book 
[Hands-On Machine Learning with Scikit-Learn and TensorFlow](https://github.com/ageron/handson-ml), an excellent resource to anyone delving into the world of machine learning and data science. The book concentrates on implementations of machine learning using Python, a general-purpose language that has a much wider audience and reach, compared to R. Having arrived a bit later than R into data science, Python's modules for machine learning are perhaps better built. In contrast to the ad hoc development of a variety of packages that have different syntax and implementations, Python's modules for machine learning are a self-contained universe for data science, able to easily implement production-ready coding pipeline that clean, model, and predict.

In the Titanic Competition, the goal is to predict who survived the tragedy. There are two data sets, a train set and a test set, already split by Kaggle for us. Normally we would split them ourselves using simple random sampling or stratified sampling, balancing out proportions and representability of not only the outcome but also of important predictors. In this case, we trust that Kaggle did that work for us, and it has to be so, otherwise different competitors would be predicting different data and results could not be compared. 

The  train set has a label, the outcome variable (survived or not) on which we conduct our supervised machine learning task, and train our algorithms to detect survival. The test set has no such label, it is our task to provide that label (the prediction). Kaggle evaluates these predictions against the truth and gives us a score. 
.	Classification and accuracy

This type of task (predicting on a binary outcome) is called a classification task, and the machine-learning algorithm we build is called a classifier. This classifier needs to be evaluated according to some performance metric. A common and intuitive performance metric, and the one we are aiming for, is called *accuracy*, which is the number of times we predict the correct outcome (survived, or not) over all the predictions we made. There are many measures of performance that could be chosen: a data analyst carefully chooses a metric, given a problem. PR (precision-recall) curves and ROC graphs (true positive rate vs. false positive rate) are often more informative in business settings where one has to weigh cost benefits of various potential implementations of a machine-learning solution. Classification performance metrics stem from different ways to consider and partition the  [confusion matrix](https://en.wikipedia.org/wiki/Confusion_matrix). 


## Making sense of the Titanic Competition

This competition is purely for learning and based on an honor code of not looking up results in the historical record, which many participants fail to observe as there are numerous "perfect scores," implying a fair amount of cheating. As a side note, it makes no particular sense to predict survival in the Titanic, except perhaps as a socio-economic or psychological study. The reason I say that a perfect score almost certainly implies cheating is that, as far as I can tell from experience and research online (of too numerous sources to cite), there is an amount of irreducible error in this survival prediction task, given the training data. This means that there is a limit to the amount of information we can extract from the data, in order to predict survivability, and it is well below 100%. 

The **drive for highest accuracy** that this Kaggle competition encourages is not necessarily a worthy goal. Most beginners in data science will be seduced by the highest score (higher accuracy) yet most algorithms that produce the highest scores (gbm, xgboost, and the like) are also well-known for overfitting. This fact, coupled with the ability to submit many predictions for the same test set, lead to the conclusion that many of the scores seen in the Titanic competition are overfitting the test set, in other words, they do not generalize to new data. Of course there is no new data, because this particular prediction has little value in a business setting, we are not going to try to inform future Titanic passengers of how to improve their chances of surviving, so it doesn't actually matter, but it would matter if you were a serious machine-learning specialist working on a realistic project that would take on new data.

My approach will be to use this introductory competition to understand the data, and emulate a production environment with pipelines in Python that clean, model, and predict on the test set in what hopefully is a robust, generalizable manner.
