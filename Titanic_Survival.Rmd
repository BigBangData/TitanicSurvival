...
Kaggle is a platform for data science, hosting data, competitions and more. The Titanic Competition is designed as an introduction to Kaggle competitions, machine-learning, or both. In the Titanic disaster, 1502 people died and 722 survived. The baseline survival rate therefore is 32.46%.

In the competition, we are given a training dataset with labels ("Survived": 1,0 where 1 represents survival) and a test dataset to predict on, with the same features (columns, or attributes) as the training set, but no label. The hypothesis is that passenger characteristics such as gender, age, and so forth, carry information that has predictive power for the outcome. This hypothesis is valid, yet only to a certain degree (more on this below).

Since the test set has 418 rows, we are not being asked to predict every passenger's outcome, but just the outcome of 418 passengers, or rather, we predict the survival of about 136 people. One simple way to predict survival is just observe attributes in which a majority survived, such as 'women', 'children', and the 'upper class' (i.e. 1st class passengers), and predict survival for them, and not for the other categories.

In this project, I will explore the data, create some informal hypotheses to help modeling, build models from simple baselines to ever more complex ones, evaluating them on their accuracy, and implement a production-ready pipeline to deploy the the most accurate model.
