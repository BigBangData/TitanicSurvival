...
Since both `SibSp` and `Parch` have skewed distributions (as we will see in the Univariate Graphical EDA section) we create Log versions of them, and then convert them to factors for plotting and EDA.


```{r}
# Logs of SibSp and Parch
train$SibSpLog <- log(train$SibSp)
train$ParchLog <- log(train$Parch)
...
After cleaning `Ticket` (see Appendix), the distribution is as follows:
...
Since `Fare` has a skewed distribution (as we will see in the Univariate Graphical EDA section) we create a `FareLog` variable which will help with linear models.
...
We just the letters with actual port names for ease in plotting.
...
[NOTE: REDO THIS SECTION ENTIRELY, USE AGE AS CONTINUOUS, RECONSIDER DISCRETE CASE LATER]

Pre-Processing `Age` had to wait until the end since we will be doing some modeling to impute missing values and needed other variables to be relatively clean before the modeling stage.

The `Age` variable can be treated as continuous or discrete. Before moving on with Graphical EDA I am creating an `AgeFac` variable that bins ages into the following groups: `Child` ($0-12$) `Teen` ($13-19$), `YoungAdult` ($20-35$), `MiddleAged` ($36-55$), and `Elderly` ($56-80$). 


## Imputing Age

Since there are many missing values, we need to explore an imputation strategy. The simple method of using a mean or median age would unnecessarily inflate the `YoungAdult` variable, as seen in the right plot below.
